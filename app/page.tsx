...
export default async function Home() {
  ...
  const collectionsResults = await Promise.all(
    collections.map(async (c) => {
      ...
      const items = res.content ? await (await import("./lib/brevozaConfig")).fetchAllItemsForCollection(c.name, res.content) : { error: "No collection config content" };
                    ...
                    <div className="mt-4">
                      ...
                      {(col as any).items?.error ? (
                        ...
                        <div className="mt-2 flex flex-col gap-3">
                          {(col as any).items.items.map((it: any) => (
                            <div key={it.path} className="rounded-md border border-zinc-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-900">
                              <div className="text-sm font-medium">{it.name}</div>
                              {it.error ? (
                                <div className="mt-1 text-xs text-red-600">{it.error}</div>
                              ) : (
                                <pre className="mt-2 max-h-[30vh] overflow-auto rounded-md bg-zinc-100 p-2 text-xs leading-5 text-zinc-900 dark:bg-zinc-900 dark:text-zinc-50">{it.content}</pre>
                              )}
                            </div>
                          ))}
                        </div>
